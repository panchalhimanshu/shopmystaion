    ...
    <SearchIcon className='text-gray-500' size={19}/>
      ...
      <div className="mb-4 grid grid-cols-1 lg:grid-cols-2 gap-x-10">
        ...
        {Object.keys(tempSearchFields).map(field => (
          <div key={field} className="flex items-center mb-2">
            ...
            {field === 'Is Published' || field === 'Sub Category'  ? (
      <select
      ...
      <div className='flex justify-between gap-1 pb-3'>
        ...
        {/* <Button color="success" className="shadow-md "><DownloadIcon size={20} className='pr-1' />Import</Button>
          ...
          <Button color="warning" className="shadow-md" onClick={()=>router.push("/admin/Catalogue/Category/categoryadd")}><Plus size={20} className='pr-1' />Add</Button>
        </div>
      </div>
      ...
      <table className="min-w-full text-left">
        ...
        <tr>
            ...
            PUBLISHED {sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
            DISPLAYORDER {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
        ...
        <tbody>
          {slicedData.map((item) => (
            <tr key={item.id}>
              ...
              <td>
                <div>
                <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-gray-700  dark:text-white"onClick={()=>router.push("/admin/Catalogue/Category/viewCategory")}>
                  <Eye size={15}></Eye>
                </Button>
                <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-gray-700  dark:text-white" onClick={()=>router.push("/admin/Catalogue/Category/EditCategory")}>

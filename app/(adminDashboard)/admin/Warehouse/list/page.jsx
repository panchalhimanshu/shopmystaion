...
import { Download, DownloadIcon, Eye, FilePenLine, FileSearch, Plus, Search, SearchIcon, Trash, Upload } from 'lucide-react';
    ...
    <div className="container mx-auto mb-3">
      ...
      <div className='flex justify-between gap-1 pb-3'>
        ...
        <div>
          ...
          <Button color="warning" className="shadow-md" onClick={() => router.push("/admin/Warehouse/list/addWareHouse")}><Plus size={20} className='pr-1' />Add</Button>
        </div>
      </div>
        ...
        <thead>
          <tr>
            ...
            <th className="px-2 py-2 cursor-pointer" onClick={() => handleSort('title')}>
              NO.OF STATIONS {sortConfig.key === 'title' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className="px-2 py-2 cursor-pointer" onClick={() => handleSort('completed')}>
              LOCATION{sortConfig.key === 'completed' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className="px-2 py-2 cursor-pointer" onClick={() => handleSort('completed')}>
              MANAGER {sortConfig.key === 'completed' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className="px-2 py-2 cursor-pointer" onClick={() => handleSort('completed')}>
        ...
        <tbody>
          {slicedData.map((item) => (
            <tr key={item.id}>
              ...
              <td>
                <div>
                  <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-gray-700  dark:text-white" onClick={() => router.push("/admin/Warehouse/list/warehouseDeatil")}>
                    <Eye size={15}></Eye>
                  </Button>
                  <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-gray-700  dark:text-white" onClick={() => router.push("/admin/Warehouse/list/editWarehouse")}>

...
import { DownloadIcon, Eye, FilePenLine, Plus, Search, SearchIcon, Trash, Upload } from 'lucide-react';
import { useRouter } from 'next/navigation';
import DeleteCommonComp from '@/components/DeleteCommonComp'
...
const list = () => {
  ...
  return (
    <>
      <div className="container mx-auto mb-3">
        ...
        <div className='flex justify-between gap-1 pb-3'>
          ...
          <div>
            ...
            <Button color="warning" className="shadow-md" onClick={() => router.push("/admin/station/list/addStation")}><Plus size={20} className='pr-1' />Add</Button>
          </div>
        </div>
          ...
          <thead>
            <tr>
              ...
              <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('username')}>
                WAREHOUSE {sortConfig.key === 'username' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('email')}>
                LOCATION {sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                MANAGER {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                STATUS {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                ONLINE<br />STATUS {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                LASTLOGIN {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
              </th>
              <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
              ...
              <tr key={item.id}>
                ...
                <td>
                  <div>
                    ...
                    <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-gray-700  dark:text-white" onClick={() => router.push("/admin/station/list/stationDetail")}>
                      <Eye size={15}></Eye>
                    </Button>
                    <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-gray-700  dark:text-white" onClick={() => router.push("/admin/station/list/editStation")}>
                      <FilePenLine size={15}></FilePenLine>
                    </Button>
                    <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-gray-700  dark:text-white" onClick={()=>setShowModal(true)}>
                      <Trash size={15}></Trash>
                    </Button>
                  </div></td>
              </tr>
      ...
      <DeleteCommonComp showmodal={showmodal} setShowModal={setShowModal} />
    </>
  );
};

  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Adding mock data for role and status
   ...
   for (let key in searchFields) {
     if (searchFields[key] !== "") {
       ...
       if (key === "status" || key === "role") {
         // Exact match for status and role
  ...
  return (
    <div className="container mx-auto">
      ...
      <div className="flex justify-center lg:mb-1 mb-3 items-center">
        <Button className="shadow-md w-28" onClick={handleSearch}>
      ...
      <div className="flex justify-between gap-1 pb-3">
        ...
        <div className="">
          <Button color="success" className="shadow-md ">
            <DownloadIcon size={20} className="pr-1" />
            Import
          </Button>
          <Button color="destructive" className="shadow-md my-2 lg:mx-2 mx-2">
            <Upload size={20} className="pr-1" />
            Export
          </Button>
          <Link href="/warehouse/warehouses/staff/addemployee">
          ...
          {slicedData.map((item) => (
            <tr key={item.id}>
              ...
              <td className="px-2 py-2">
                <Badge color={getBadgeColor(item.role)}>{item.role}</Badge>
              </td>{" "}
              <td
                className={`px-2 py-2 ${getStatusColor(
              ...
              <td className="px-4 py-2">
                <div>
                  <Link
                    href={`/warehouse/warehouses/staff/viewemployee?id=${item.id}`}
                  >
                  ...
                  <Link
                    href={`/warehouse/warehouses/staff/editemployee?id=${item.id}`}
                  >
                    <Button className="p-0 mr-2 bg-transparent hover:bg-transparent text-black dark:text-white">

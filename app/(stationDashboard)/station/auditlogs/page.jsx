      ...
      <div className="mb-4 grid grid-cols-1 lg:grid-cols-2 gap-x-10">
        ...
        {Object.keys(tempSearchFields).map((field) => (
          <div key={field} className="flex items-center mb-2">
            ...
            {field === "CreatedFrom" || field === "Created To" ? ( // Check if field is "CreatedFrom" or "Created To"
              <input
              ...
              />
            ) : field === "Log Leve" ? ( // Check if field is "Log Level"
              <select
                className="border border-gray-300 px-4 py-2 rounded w-3/4"
                value={tempSearchFields[field]}
                onChange={(e) => handleInputChange(field, e.target.value)}
          ...
          {slicedData.map((item) => (
            <tr key={item.id}>
              ...
              <td className="px-4 py-2">
                ...
                <Link href={"/station/auditlogs/viewauditlogs"}>    <Button className="p-0 mr-2 bg-transparent text-black  dark:text-white hover:bg-transparent ">
                    <Eye size={20}></Eye>
                  </Button> </Link>  
                </div>
              </td>

...
const getTopProducts = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/GetTopProduct?OrgId=${userData?.orgid}&UaId=${userData?.uaid}&From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}&Orderby=2&NoRecords=10&sortby=true`, 'GET', null, 'Auth');
                ...
                <thead>
                    <tr>
                        ...
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('username')}>
                            Product Image{sortConfig.key === 'username' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        ...
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Product Price {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                        ...
                        // data?.length > 0 ?
                            data?.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-2 py-2">{item?.proimage}</td>
                                    <td className="px-2 py-2">{item?.pvname}</td>
                                    <td className="px-2 py-2">{item?.pvalue}</td>
                                    <td className="px-2 py-2">{item?.pqty}</td>
                                </tr>
                            ))
                            // :
